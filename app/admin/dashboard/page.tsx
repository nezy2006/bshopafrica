...
import type { AdminOverview, AdminOrder, AdminClient } from "@/lib/whmcs";
...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    Promise.all([
      whmcsAdmin<AdminOverview>("adminGetOverview"),
    ...
    <div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4 mb-8">
        {loading ? Array.from({ length: 6 }).map((_, i) => (
          ...
          <StatCard label="Total Clients"   value={stats?.clientsTotal ?? 0}         color="purple" sub="registered" icon={<Icon d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2 M9 7a4 4 0 1 0 0-8 4 4 0 0 0 0 8z"/>}/>
          <StatCard label="Active Hosting"  value={stats?.hostingActive ?? 0}        color="blue"   sub="services"    icon={<Icon d="M6 2 3 6v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2V6l-3-4z M3 6h18 M16 10a4 4 0 0 1-8 0"/>}/>
          <StatCard label="Active Domains"  value={stats?.domainsActive ?? 0}        color="orange" sub="registered"  icon={<Icon d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/>}/>
          <StatCard label="Revenue/Month"   value={`$${(stats?.monthlyRevenue ?? 0).toFixed(2)}`} color="green" sub="USD" icon={<Icon d="M12 2v20 M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6"/>}/>
          <StatCard label="Open Tickets"    value={stats?.ticketsOpen ?? 0}          color="red"    sub="need reply"  icon={<Icon d="M2 9a3 3 0 0 1 0 6v2a2 2 0 0 0 2 2h16a2 2 0 0 0 2-2v-2a3 3 0 0 1 0-6V7a2 2 0 0 0-2-2H4a2 2 0 0 0-2 2z M9 12h6"/>}/>
          <StatCard label="Pending Orders"  value={stats?.ordersPending ?? 0}        color="purple" sub="awaiting"   icon={<Icon d="M6 2 3 6v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2V6l-3-4z M3 6h18 M16 10a4 4 0 0 1-8 0"/>}/>
        </>)}
      </div>

      {/* Charts */}
      <div className="grid lg:grid-cols-2 gap-6 mb-8">
        <BarChart data={stats?.revenueByDay ?? []} label="Monthly Revenue (USD)" />
        <BarChart data={stats?.ordersByDay ?? []}  label={`Orders This Week (${stats?.ordersThisWeek ?? 0})`} />
      </div>
